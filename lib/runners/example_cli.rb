...
@@simplified_genres = ['Blues', 'Classical','Country','Electronic','Hip Hop',
'Industrial','Jazz','Pop','R&B','Rock','World']
   ...
   while input
    ...
    when "search"
      puts "Enter a genre:"
        @input = get_user_input
        search
        puts "Artist popularity is measured from 1-100. Please enter a popularity range separated by a dash ('i.e. 40-80')"
        @input = get_user_input
        popularity_filter
        return_names
    when "genre"
  ...
  def popularity_filter
    ...
    popularity_input_one = input_arr[0].to_i
    popularity_input_two = input_arr[1].to_i
    ...
    artist_data.each do | artist, pop |
      pop.each do |k,v|
        if v >= popularity_input_one && v <= popularity_input_two
          @@pop_arr << artist
 ...
 #broke this into a separate method
  ...
  def list_genres
    @@simplified_genres.each_slice(3) do |a,b,c|
        puts [a,b,c]
      end
    end
